...
DROP TABLE IF EXISTS fastag_transfers;
DROP TABLE IF EXISTS ecom_orders;
DROP TABLE IF EXISTS ecom_leads;
...
-- user_sessions (activity/heartbeat tracking)
...
CREATE TABLE dispatch_order_items (
  ...
  CONSTRAINT fk_doi_order FOREIGN KEY (order_id)
    REFERENCES dispatch_orders(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- fastag_transfers (inventory transfer log)
